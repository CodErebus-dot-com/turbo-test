...
const { fileYielder } = require('./helpers');
...
function copyDir(sourcePath, destPath, exclusions) {
  // renaming files ending with __template__ with ''
  for (const filePath of fileYielder(sourcePath)) {
    if (filePath.endsWith('__template__')) {
      renameSync(filePath, filePath.replace('__template__', ''));
    }
  }
...
function writeJsonFile(jsonFilePath, json) {
  try {
    ...
    console.error(
      chalk.red(`[${currentDateTime(new Date())}] - Error copying file ${jsonFilePath}. ${e}`),
    );
    throw e;
  }
}
