...
function runCommand(commandType, ...args) {
  ...
  console.info(
    `[${chalk.green(currentDateTime())}] - ${emoji.get(
      'rocket',
  ...
  execCommand.stderr.on('data', data => {
    if (data.includes('warning')) {
      console.error(`[${chalk.yellow(currentDateTime())}] - ${chalk.yellow(data)}`);
    } else {
      console.error(`[${chalk.yellow(currentDateTime())}] - ${chalk.red(data)}`);
    }
  });

  execCommand.on('exit', () => {
    console.info(
      `[${chalk.green(currentDateTime())}] - ${emoji.get('tada')} Command execution completed.`,
    );
    spinnerInit.stop();
  });
}
...
function installPackages(commandType, newProject, stepIn, isRecentDir) {
  ...
  console.info(
    chalk.cyan.bold(
      `[${currentDateTime()}] - ${emoji.get('bulb')} Using [${commandName}] as package manager.`,
    ),
  );
  console.info(
    chalk.cyan.bold(
      `[${currentDateTime()}] - ${emoji.get('rocket')} Start Installing dependencies.`,
    ),
  );
  spinnerInit.start();
  ...
  installDependencies.stdout.on('data', data => {
    console.info(`[${chalk.green(currentDateTime())}] - ${data}`);
  });
  ...
  installDependencies.on('exit', () => {
    ...
    console.info(
      `[${chalk.green(currentDateTime())}] - ${emoji.get('+1')} Installing dependencies completed.`,
    );

    if (newProject) {
      console.info(
        `[${chalk.green(currentDateTime())}] - ${emoji.get('tada')} ${emoji.get(
          'clap',
...
export = { installPackages, runCommand };
